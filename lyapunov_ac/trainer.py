"""Train LAC entirely by episodes and record every episode."""
...
def scheduled_learning_rate(initial, final, episode, episodes, decay_type):
    """Match the repository's epoch decay using episodes as the reference."""
    if decay_type == "constant" or episodes == 1:
        return initial
...
def evaluate(lac_agent, env, episodes):
    costs = []
    for test_episode in range(1, episodes + 1):
        ...
        while not (terminated or truncated):
            action = lac_agent.get_action(obs, deterministic=True)
            obs, cost, terminated, truncated, _ = env.step(action)
            total_cost += cost
...
def train(train_config):
    ...
    env = envs.make_cost_env(train_config.env_id, train_config.seed)
    test_env = envs.make_cost_env(
        train_config.env_id, train_config.seed + 1
    )
    lac_agent = agent.LACAgent(
    ...
    model_path = run_dir / "lac_model.pt"
    episode_fields = [
        "episode",
        "total_steps",
        "episode_reward",
    ...
    print("LAC network: stochastic Actor + Lyapunov Critic")
    print(f"Training episodes: {train_config.episodes}")
    print(f"Device: {lac_agent.device}")
    ...
    with episodes_path.open(
        ...
        for episode_number in range(1, train_config.episodes + 1):
            current_learning_rates = {
                "actor_lr": scheduled_learning_rate(
                    train_config.actor_lr,
                    train_config.actor_lr_final,
                    episode_number,
                    train_config.episodes,
                    train_config.lr_decay_type,
                ),
                "critic_lr": scheduled_learning_rate(
                    train_config.critic_lr,
                    train_config.critic_lr_final,
                    episode_number,
                    train_config.episodes,
                    train_config.lr_decay_type,
                ),
                "alpha_lr": scheduled_learning_rate(
                    train_config.alpha_lr,
                    train_config.alpha_lr_final,
                    episode_number,
                    train_config.episodes,
                    train_config.lr_decay_type,
                ),
                "labda_lr": scheduled_learning_rate(
                    train_config.labda_lr,
                    train_config.labda_lr_final,
                    episode_number,
                    train_config.episodes,
                    train_config.lr_decay_type,
                ),
            }
            lac_agent.set_learning_rates(**current_learning_rates)
            ...
            while not (terminated or truncated):
                if total_steps < train_config.start_steps:
                    ...
                    action = lac_agent.get_action(obs)

                obs_next, cost, terminated, truncated, _ = env.step(action)
                memory.store(
                    obs,
                    action,
                    cost,
            ...
            if should_evaluate:
                test_cost = evaluate(
                    lac_agent, test_env, train_config.test_episodes
                )
            else:
                test_cost = float("nan")
            ...
            row["alpha"] = float(lac_agent.alpha.detach().cpu())
            row["labda"] = float(lac_agent.labda.detach().cpu())
            writer.writerow(row)
            episodes_file.flush()
            plotting.save_reward_curve(
                ...
                title=f"LAC on {train_config.env_id}",
            )
            lac_agent.save(model_path)
            ...
            print(
                ...
                f"lambda {row['labda']:.4f} | "
                f"actor lr {row['actor_lr']:.2e}"
            )
            if should_evaluate:
                print(f"  Evaluation | cost {test_cost:10.3f}")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train episode-based Lyapunov Actor-Critic."
    )
    ...
    parser.add_argument("--labda", type=float, default=0.99)
    parser.add_argument("--actor-lr", type=float, default=1e-4)
    parser.add_argument("--critic-lr", type=float, default=3e-4)
    parser.add_argument("--alpha-lr", type=float, default=1e-4)
    parser.add_argument("--labda-lr", type=float, default=3e-4)
    parser.add_argument("--actor-lr-final", type=float, default=1e-10)
    parser.add_argument("--critic-lr-final", type=float, default=1e-10)
    parser.add_argument("--alpha-lr-final", type=float, default=1e-10)
    parser.add_argument("--labda-lr-final", type=float, default=1e-10)
    parser.add_argument(
        "--lr-decay-type",
...
def main():
    ...
    train_config = config.TrainConfig(
        ...
        actor_lr_final=args.actor_lr_final,
        critic_lr_final=args.critic_lr_final,
        alpha_lr_final=args.alpha_lr_final,
        labda_lr_final=args.labda_lr_final,
