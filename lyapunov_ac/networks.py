"""Actor and Lyapunov critic used by LAC."""
...
def build_mlp(
    layer_sizes, activation=nn.ReLU, output_activation=nn.ReLU
):
    layers = []
...
class SquashedGaussianActor(nn.Module):
    """Stochastic squashed Gaussian actor with entropy regularization."""
    ...
    def forward(self, obs, deterministic=False, with_logprob=True):
        ...
        log_std = torch.clamp(
            self.log_std_layer(features), -20.0, 2.0
        )
        ...
        if with_logprob:
            ...
            correction = 2.0 * (
                np.log(2.0)
                - raw_action
                - functional.softplus(-2.0 * raw_action)
            )
...
class LyapunovCritic(nn.Module):
    """Non-negative Lyapunov critic implemented as a sum of squares."""
...
class LyapunovActorCritic(nn.Module):
    def __init__(
        self, observation_space, action_space, hidden_sizes
    ):
        super().__init__()
        obs_dim = observation_space.shape[0]
        act_dim = action_space.shape[0]
        self.actor = SquashedGaussianActor(
            obs_dim, action_space, hidden_sizes
        )
        self.critic = LyapunovCritic(
            obs_dim, act_dim, hidden_sizes
        )
